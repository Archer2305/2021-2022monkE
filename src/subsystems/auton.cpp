...
//   //calculate the arc
//
//   if((x>0.5833&&y>0)){ //pick which formula to use
//   De=-1*((atan((y/(x-0.5833)))*(180/3.1415))+90);
// }else{
//    De=((atan((y/(x-0.5833))*(180/3.1415))+90));
//   }
// float	arc=(3.1415/180)*(14/12)*De; // calculate arc length now that we have degrees needed to turn
//   //ok now that the initializations are done we can do the logic part of the turn
//   if(De==0){
//     while(rightTracking.get_value()<(arc*(4320/c))){
...
//     // moveLeft(0);
//   }else if(De>0){
//   // while(rightTracking.get_value()<(arc*0.7199)){
// 		while(rightTracking.get_value()<1000){
//   pros::lcd::set_text(4, "Entered this loop");
...
//     // while(leftTracking.get_value()<(arc*0.7199)){
// 		while(leftTracking.get_value()<1000){
// pros::lcd::set_text(4, "Entered poop loop");
